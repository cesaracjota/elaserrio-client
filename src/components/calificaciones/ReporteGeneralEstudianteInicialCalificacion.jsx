...
const AcademicReportPDF = ({ studentData, evaluaciones, schoolInfo, sede }) => {
  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        {/* Tabla de experiencias y valoraciones por periodo */}
        <View style={styles.table}>
          ...
          {/* Filas de experiencias con valoraciones por periodo */}
          {evaluaciones?.map((exp, index) => {
            ...
            return (
              <View key={index}>
                {/* Fila del nombre de la materia - que abarca los 4 periodos en altura */}
                <View
                ...
                >
                  ...
                  {/* Columna para las filas de periodos */}
                  <View style={{ width: '70%' }}>
                    {/* Primer periodo */}
                    <View style={styles.periodRow}>
                      <View style={[styles.periodCell, { width: '21.5%' }]}>
                        <Text>PERIODO 1</Text>
                      </View>
                      <View style={[styles.valoracionCell, { width: '78.5%' }]}>
                        {exp.valoraciones?.find(val => val.periodo === 1) ? (
                          <Text>
                            {
                              exp.valoraciones.find(val => val.periodo === 1)
                                .valoracion
                    ...
                    <View style={styles.periodRow}>
                      ...
                      <View style={[styles.valoracionCell, { width: '78.5%' }]}>
                        {exp.valoraciones?.find(val => val.periodo === 2) ? (
                          <Text>
                            {
                              exp.valoraciones.find(val => val.periodo === 2)
                                .valoracion
                    ...
                    <View style={styles.periodRow}>
                      ...
                      <View style={[styles.valoracionCell, { width: '78.5%' }]}>
                        {exp.valoraciones?.find(val => val.periodo === 3) ? (
                          <Text>
                            {
                              exp.valoraciones.find(val => val.periodo === 3)
                                .valoracion
                    ...
                    <View style={styles.periodRow}>
                      ...
                      <View style={[styles.valoracionCell, { width: '78.5%' }]}>
                        {exp.valoraciones?.find(val => val.periodo === 4) ? (
                          <Text>
                            {
                              exp.valoraciones.find(val => val.periodo === 4)
                                .valoracion
...
const ReporteGeneralEstudianteInicialCalificacion = ({
  data,
  configuracion,
}) => {
  ...
  const generateAndDownloadPdf = async () => {
    ...
    try {
      ...
      CustomToast({
        ...
        message:
          error.message ||
          'Error al generar el boletín académico. Revise si el estudiante tiene evaluaciones registradas.',
        type: 'error',
        duration: 3000,
        position: 'top',
      });
  ...
  return (
    <Tooltip label="Generar boletín general" placement="auto">
      <Button
        colorScheme="pink"
        variant="outline"
        size="sm"
...
export default ReporteGeneralEstudianteInicialCalificacion;
